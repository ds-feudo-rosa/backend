...
class ReturnNotice(Resource):
    ...
    def post(self):
        ...
        if NewsModel.query.filter_by(id=data['id']).first():
            ...
            return ("Title = {}, Texto = {}, Likes = {}, Deslikes = {}, link da Imagem = {}".format(notice.title, notice.text, votos.like, votos.unlike, notice.url))
        else:
...
class VotoNotice(Resource):
    def get(self):
        ...
        for x in votos:
            dados = x.__dict__
            list_votos.append(dados['id'])
            list_votos.append(dados['like'])
            list_votos.append(dados['unlike'])

        return json.dumps(list_votos)  
